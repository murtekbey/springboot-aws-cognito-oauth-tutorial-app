...
public class JwtProvider {
        ...
        if (token != null) {
            ...
            if (username != null) {
                ...
                List<GrantedAuthority> authorities =
                        rolesToList(roles).stream().map(role -> new SimpleGrantedAuthority(role)).collect(Collectors.toList());
    ...
    private void validateToken(JWTClaimsSet claims) throws Exception {
        if (!claims.getIssuer().equals(applicationConfig.getIdentityPoolUrl())) {
